...
import (
	"exchange-provider/internal/delivery/exchanges/dex/multichain/contracts/AnyswapV4Router"
...
func anySwapOutUnderlying(in, out *Token, amount *big.Int, src common.Address,
	...
	case "AnyswapV4Router", "AnyswapV3Router":
		c, err := AnyswapV4Router.NewContracts(common.HexToAddress(in.Data.Router), pr.Client)
		if err != nil {
			return nil, nil, err
		}
		...
		tx, err := c.AnySwapOutUnderlying(opts, common.HexToAddress(in.Data.FromAnyToken.Address), src,
			amount, big.NewInt(int64(destChain)))
	...
	case "AnyswapV6Router", "MultichainV7Router":
		...
		tx, err := c.AnySwapOutUnderlying(opts, common.HexToAddress(in.Data.FromAnyToken.Address), src.String(),
			amount, big.NewInt(int64(destChain)))
		return tx, opts.Nonce, err
